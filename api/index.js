...
const salasRouter = require('../backend/routes/salas');
const alunosRouter = require('../backend/routes/alunos');
const chamadaRouter = require('../backend/routes/chamada');
        ...
        db.all('SELECT * FROM responsaveis ORDER BY id', [], (err, rows) => {
          ...
          db.all('SELECT * FROM enderecos ORDER BY id', [], (err, rows) => {
            ...
            db.all('SELECT * FROM frequencias ORDER BY data, crianca_id', [], (err, rows) => {
              ...
              res.json({ exportado_em: new Date().toISOString(), turmas, criancas, responsaveis, enderecos, frequencias });
            });
          });
        });
      });
...
app.post('/api/import', (req, res) => {
  ...
  db.serialize(() => {
    ...
    db.run('DELETE FROM frequencias'); db.run('DELETE FROM enderecos');
    db.run('DELETE FROM responsaveis'); db.run('DELETE FROM criancas'); db.run('DELETE FROM turmas');
    if (turmas && Array.isArray(turmas)) {
      const stmt = db.prepare('INSERT INTO turmas (id, nome, professor, created_at) VALUES (?,?,?,?)');
      turmas.forEach(t => stmt.run(t.id, t.nome, t.professor||null, t.created_at||new Date().toISOString(), function(err) { if (!err) count.turmas++; else count.erros++; }));
      stmt.finalize();
    }
    const cStmt = db.prepare('INSERT INTO criancas (id, nome, data_nascimento, sexo, turma_id, observacoes, created_at) VALUES (?,?,?,?,?,?,?)');
    criancas.forEach(c => cStmt.run(c.id, c.nome, c.data_nascimento||null, c.sexo||null, c.turma_id||null, c.observacoes||null, c.created_at||new Date().toISOString(), function(err) { if (!err) count.criancas++; else count.erros++; }));
    cStmt.finalize();
...
app.get('/api/estatisticas', (req, res) => {
  const db = require('../backend/database');
  db.all(`SELECT f.data, COUNT(CASE WHEN f.status = 'presente' THEN 1 END) as presentes,
    ...
    FROM frequencias f WHERE f.data >= date('now', '-30 days')
    GROUP BY f.data ORDER BY f.data`, [], (err, rows) => {
    if (err) return res.status(500).json({ error: err.message });
    ...
    res.json({ dias: rows || [], total_presentes: totalP, total_ausentes: totalA, media_presenca: total > 0 ? Math.round((totalP / total) * 100) : 0 });
  });
});
