      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="c in criancas" :key="c.id">
            ...
            <td>
              <button v-if="!c._statusHoje || c._statusHoje === ''" class="btn-sm btn-green" @click="registrar(c, 'presente')">✓ Presente</button>
              <button v-if="!c._statusHoje || c._statusHoje === ''" class="btn-sm btn-orange" @click="registrar(c, 'ausente_justificada')">📋</button>
              <button v-if="!c._statusHoje || c._statusHoje === ''" class="btn-sm btn-red" @click="registrar(c, 'ausente_nao_justificada')">✗</button>
...
async function carregar() {
  ...
  const [cResp, tResp, fResp] = await Promise.all([
    ...
    api.getFrequencias({ data: dataFiltro.value, turma_id: turmaFiltro.value || undefined }).catch(() => ({ data: [] }))
  ])

  turmas.value = tResp.data
  const freqs = fResp.data
  const freqMap = {}
  freqs.forEach(f => { freqs[f.crianca_id] = f })

  criancas.value = cResp.data.map(c => {
    const f = freqMap[c.id]
    return {
      ...c,
      _statusHoje: f ? f.status : '',
      _presentes: 0,
      _justificadas: 0,
      _nao_justificadas: 0,
      _percentual: 0
...
.progress-bar { width: 60px; height: 6px; background: #F3F4F6; border-radius: 3px; }
.progress-fill { height: 100%; border-radius: 3px; transition: width 0.5s ease; }

.btn-sm { padding: 5px 12px; border: 1px solid; border-radius: 6px; font-size: 0.78rem; font-weight: 500; cursor: pointer; font-family: inherit; transition: var(--transition); margin-right: 4px; }
